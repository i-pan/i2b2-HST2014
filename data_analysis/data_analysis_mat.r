...
year.bins <- 10
case <- cbind(case, 
              age_bin = as.numeric(cut(as.numeric(case[, 'age']), 
							                         breaks = seq(0, 11*year.bins, 
																			              by = year.bins), 
																			 labels = c(seq(0, year.bins, by = 1)), 
	                                     right = TRUE)))
cont <- cbind(cont, 
              age_bin = as.numeric(cut(as.numeric(cont[, 'age']), 
							                         breaks = seq(0, 11*year.bins, 
																						        by = year.bins), 
																			 labels = c(seq(0, year.bins, by = 1)), 
	                                     right = TRUE)))
...
# create facts bins 
fact.bins <- 20 
fact.qtile <- c(quantile(as.numeric(case[, 'num_facts']), probs = seq(0, 1, by = 1/fact.bins)))
case <- cbind(case, 
              fact_bin = as.numeric(cut(as.numeric(case[, 'num_facts']), 
							                          breaks = fact.qtile, 
																				labels = seq(1, fact.bins, 
																							       by = 1), 
																				include.lowest = TRUE)))
cont <- cbind(cont, 
							fact_bin = as.numeric(cut(as.numeric(cont[, 'num_facts']), 
							                          breaks = fact.qtile, 
																				labels = seq(1, fact.bins, 
																							       by = 1), 
																				include.lowest = TRUE)))
...
case <- case[order(case[, 'gender'], case[, 'race'], case[, 'age_bin'], 
									 case[, 'fact_bin'], case[, 'num_facts']), ]
cont <- cont[order(cont[, 'gender'], cont[, 'race'], cont[, 'age_bin'], 
									 cont[, 'fact_bin'], cont[, 'num_facts']), ]
...
case.cont.dx <- matrix(nrow = 0, ncol = ncol(dx))

for(each.ID in case.cont.ID) {
  case.cont.dx <- rbind(case.cont.dx, 
	                      dx[dx[, 'patient_num'] == each.ID, ])
}
...
for(each.code in case.cont.dx[, 2]) {
  print(paste("Retrieving PheWAS code for row", index, "..."))
...
for(i in 1:nrow(case.cont.dx)) {
  print(paste("Operating on row", i, "..."))
...
for(each.dis in 1:dis.matrix.col) {
	dis.incid.case[each.dis] <- sum(as.numeric(dis.matrix.case[, each.dis+1]))
	dis.incid.cont[each.dis] <- sum(as.numeric(dis.matrix.cont[, each.dis+1]))
}
...
for(i in 1:dis.matrix.thres.col) {
	...
	fit <- glm(as.numeric(case.cont.data[, i+11]) ~ as.numeric(case.cont.data[, 'status']) + 
																			as.numeric(case.cont.data[, 'num_facts']), 
						 family=binomial) # facts in model 
	fit2 <- glm(as.numeric(case.cont.data[, i+11]) ~ as.numeric(case.cont.data[, 'status']),  
...
for(i in 1:dis.matrix.thres.col) {
	print(paste("Fitting conditional logistic regression for disease", i, 
		"..."))
	cond.fit <- clogit(as.numeric(case.cont.data[, i+11]) ~ as.numeric(case.cont.data[, 'status']) +
                                              as.numeric(case.cont.data[, 'num_facts']),	

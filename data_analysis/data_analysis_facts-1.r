...
# adjust facts
tmp.cont.facts <- cont$fact_bin
tmp.case.facts <- case$fact_bin
case$fact_bin <- ifelse(tmp.case.facts == 20, tmp.case.facts, tmp.case.facts+1)
...
for(each.case in 1:nrow(case)) {
	print(paste("Matching case", each.case, 
							"based on gender, race, exact age, fact bin ..."))
...
colnames(dis.matrix.thres) <- gsub(" ", "", colnames(dis.matrix.thres))
...
for(i in 1:dis.matrix.thres.col) {
	print(paste("Fitting conditional logistic regression for disease", i, 
		"..."))
	cond.fit <- clogit(dis.matrix.thres[, i+2] ~ dis.matrix.thres[, 1] +
                                               dis.matrix.thres[, 2] +
																							 dis.matrix.thres[, 1]*dis.matrix.thres[, 2],	
		                                          strata(case.cont.data$fact_bin))
...
hist(odds.nofacts, col=rgb(0,0,1,1/4), prob=T, breaks=10, add=T)
lines(density(odds.nofacts), col=rgb(0,0,1,1), lwd=3)
hist(cond.odds, col=rgb(0,1,0,1/4), prob=T, breaks=10, add=T)
